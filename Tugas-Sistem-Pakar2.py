...
def show_gejala(gejala):
    ...
    for nama_gejala, data in gejala.items():
        print(f"{nama_gejala} - {data}")

    gejala_user = input("Masukan Gejala dengan Spasi :").upper().split()
...
def cari_penyakit(penyakit, gejala_user):
    ...
    for nama_penyakit, data in penyakit.items():
        gejala_penyakit = data
        skor = 0

        for gejala in gejala_penyakit:
            if gejala in gejala_user:
                skor += 1
        
        hasil_penyakit.append((nama_penyakit, skor))
    
    return max(hasil_penyakit, key=lambda x: x[1])

gejala_user = show_gejala(gejala)
hasil_diagnosa = cari_penyakit(penyakit, gejala_user)
